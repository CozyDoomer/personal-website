...
def IoU_values(anchors, targets):
    ...
    anc_sz, tgt_sz = anchors[:, 2] * \
        anchors[:, 3], targets[:, 2] * targets[:, 3]
...
def draw_rect(ax: plt.Axes, b: Collection[int], color: str = 'white', text=None, text_size=14):
    ...
    if text is not None:
        patch = ax.text(*b[:2], text, verticalalignment='top',
                        color=color, fontsize=text_size, weight='bold')
...
def show_results_side_by_side(learn: Learner, anchors, detect_thresh: float = 0.2, nms_thresh: float = 0.3,  image_count: int = 5):
    ...
    with torch.no_grad():
        img_batch, target_batch = learn.data.one_batch(
            DatasetType.Valid, False, False, False)
        ...
        for img, bbox_gt, class_gt, clas_pred, bbox_pred in list(
            ...
            show_results(img, bbox_pred, preds, scores, learn.data.train_ds.classes[1:], bbox_gt, class_gt, (
                15, 15), titleA="GT", titleB="Prediction")


def show_results(img, bbox_pred, preds, scores, classes, bbox_gt, preds_gt, figsize=(5, 5), titleA: str = "", titleB: str = ""):
    ...
    if bbox_pred is not None:
        for bbox, c, scr in zip(bbox_pred, preds, scores):
            ...
            draw_rect(ax[1], [bbox[1], bbox[0], bbox[3],
                              bbox[2]], text=f'{txt} {scr:.2f}')
...
def draw_bbox_on_orig(original_image_path, transformed_image, scores, bbox_preds, class_names):
    ...
    for score, bbox_pred, class_name in zip(scores, bbox_preds, class_names):
        ...
        draw.rectangle((bbox_pred[0] - 1, bbox_pred[1] - 1,
                        bbox_pred[2] + 1, bbox_pred[3] + 1), outline='black', width=4)
        draw.rectangle((bbox_pred[0],     bbox_pred[1],     bbox_pred[2],
                        bbox_pred[3]),     outline='white', width=2)
...
def filter_predictions(tfm_image, classes, bboxes, anchors, thresh=0.3):
    ...
    bbox_preds, scores, preds = process_output(
        classes, bboxes, anchors, detect_thresh=thresh)
    ...
    bbox_preds, preds, scores = bbox_preds[to_keep].cpu(
    ), preds[to_keep].cpu(), scores[to_keep].cpu()
